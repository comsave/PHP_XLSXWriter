...
class XLSXWriter
{
    ...
    public function writeToFile($filename)
    {
        ...
        $zip->addFile($this->writeStylesXML(), "xl/styles.xml");  //$zip->addFromString("xl/styles.xml"           , $this->buildStylesXML() );
    ...
    {
        $number_format_idx = $this->add_to_list_get_index($this->number_formats, $number_format);
        $lookup_string = $number_format_idx . ";" . $cell_style_string;
        $cell_style_idx = $this->add_to_list_get_index($this->cell_styles, $lookup_string);
    ...
    private function initializeColumnTypes($header_types)
    {
        ...
        foreach ($header_types as $v) {
            $number_format = $this->numberFormatStandardized($v);
    ...
    protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
    {
        ...
        if (!is_scalar($value) || $value === '') { //objects, array, empty
            ...
            $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="s"><f>' . $this->xmlspecialchars($value) . '</f></c>');
        } elseif ($num_format_type == 'n_date') {
            $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . intval($this->convert_date_time($value)) . '</v></c>');
        } elseif ($num_format_type == 'n_datetime') {
            $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . $this->convert_date_time($value) . '</v></c>');
        } elseif ($num_format_type == 'n_numeric') {
            $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . $this->xmlspecialchars($value) . '</v></c>');//int,float,currency
        } elseif ($num_format_type == 'n_string') {
            $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="inlineStr"><is><t>' . $this->xmlspecialchars($value) . '</t></is></c>');
    ...
    protected function styleFontIndexes()
    {
        ...
        foreach ($this->cell_styles as $i => $cell_style_string) {
            ...
            if (isset($style['border']) && is_string($style['border'])) {
                ...
                $style_indexes[$i]['border_idx'] = $this->add_to_list_get_index($borders, implode(",", $border_value));
            }
            if (isset($style['fill']) && is_string($style['fill']) && $style['fill'][0] == '#') {
                ...
                $style_indexes[$i]['fill_idx'] = $this->add_to_list_get_index($fills, "FF" . strtoupper($v));
            }
            ...
            if ($font != $default_font) {
                $style_indexes[$i]['font_idx'] = $this->add_to_list_get_index($fonts, json_encode($font));
            }
        }
    ...
    protected function writeStylesXML()
    {
        ...
        foreach ($this->number_formats as $i => $v) {
            $file->write('<numFmt numFmtId="' . (164 + $i) . '" formatCode="' . $this->xmlspecialchars($v) . '" />');
        }
    ...
    protected function buildWorkbookXML()
    {
        ...
        $workbook_xml .= '<sheets>';
        foreach ($this->sheets as $sheet_name => $sheet) {
            $workbook_xml .= '<sheet name="' . $this->xmlspecialchars($sheet->sheetname) . '" sheetId="' . ($i + 1) . '" state="visible" r:id="rId' . ($i + 2) . '"/>';
            $i++;
        }
        $workbook_xml .= '</sheets>';
